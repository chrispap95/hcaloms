...
parser = argparse.ArgumentParser(description='This script is used to extract the mean and rms for pedestals')
parser.add_argument('-f','--inputFile', help='Input file name', required=True)
parser.add_argument('-t','--timeStamp', help='Include run timestamp', action='store_true')
parser.add_argument('-z','--suppressZero', help='Suppress runs that give zero for all pedestals', action='store_true')
...
for depth in range(1,8):
    ...
    plot_mean = fileIn.Get(f'DQMData/Run {runNum}/Hcal/Run summary/PedestalTask/Mean/depth/depth{depth}')
    plot_rms = fileIn.Get(f'DQMData/Run {runNum}/Hcal/Run summary/PedestalTask/RMS/depth/depth{depth}')
    ...
    for i in range(xi, xf+1):
        for j in range(yi, yf+1):
            ...
            if plot_mean.GetBinContent(i+xPlusOffset, j) != 0:
                sensors['HF']['mean'].append(plot_mean.GetBinContent(i+xPlusOffset, j))
                sensors['HF']['rms'].append(plot_rms.GetBinContent(i+xPlusOffset, j))

# Fill HO pedestals
plot_mean = fileIn.Get(f'DQMData/Run {runNum}/Hcal/Run summary/PedestalTask/Mean/depth/depthHO')
plot_rms = fileIn.Get(f'DQMData/Run {runNum}/Hcal/Run summary/PedestalTask/RMS/depth/depthHO')
xi, xf, yi, yf = 14, 71, 1, 72
for i in range(xi, xf+1):
    for j in range(yi, yf+1):
        if plot_mean.GetBinContent(i, j) != 0:
