...
parser = argparse.ArgumentParser(description='This script is used to upload analysis data to oracle database')
...
for root, dirs, files in os.walk(dir_path, topdown=True):
    if data_file in files:
        if os.stat(data_path+data_file).st_size > 0 :
            upload_to_db=subprocess.Popen(['sqlldr', 'parfile='+parfile], stdout=subprocess.PIPE, stderr=subprocess.PIPE) 
